...
CREATE TABLE IF NOT EXISTS `Operator` (
  ...
  UNIQUE KEY `idx_unique_operator_contact_info` (`contact_info`)
) ENGINE = InnoDB
...
CREATE TABLE IF NOT EXISTS `Charging_station` (
  ...
  UNIQUE KEY `idx_unique_charging_station_location` (`location`)
) ENGINE = InnoDB
AUTO_INCREMENT = 8
DEFAULT CHARACTER SET = utf8mb4;
...
CREATE TABLE IF NOT EXISTS `Solar_System` (
  ...
  UNIQUE KEY `idx_unique_solar_technology_station` (`technology_used`, `station_id`)
) ENGINE = InnoDB
AUTO_INCREMENT = 8
DEFAULT CHARACTER SET = utf8mb4;
...
CREATE TABLE IF NOT EXISTS `Robot` (
  ...
  -- UNIQUE KEY `idx_unique_robot_operator_station` (`operator_id`, `station_id`)
) ENGINE = InnoDB
AUTO_INCREMENT = 10
DEFAULT CHARACTER SET = utf8mb4;
...
CREATE TABLE IF NOT EXISTS `Tasks` (
    ...
    `task_type` ENUM('Monitoring', 'Reporting', 'Inspection', 'Other') NOT NULL,
    `priority` ENUM('Low', 'Medium', 'High') NOT NULL DEFAULT 'Medium',
    `status` ENUM('Assigned', 'In Progress', 'Completed') NOT NULL DEFAULT 'Assigned',
    `created_at` DATETIME DEFAULT CURRENT_TIMESTAMP,
    `due_date` DATE NOT NULL,
    `description` TEXT
--     CONSTRAINT `fk_operator` FOREIGN KEY (`operator_id`) REFERENCES Operator (`operators_id`)
...
CREATE TABLE IF NOT EXISTS `Audio_system` (
  ...
  UNIQUE KEY `idx_unique_audio_system_robot` (`robot_id`, `status`)
) ENGINE = InnoDB
AUTO_INCREMENT = 11
DEFAULT CHARACTER SET = utf8mb4;
...
CREATE TABLE IF NOT EXISTS `Patrol_Route` (
  ...
  UNIQUE KEY `idx_unique_route_start_end` (`start_point`, `end_point`)
) ENGINE = InnoDB
AUTO_INCREMENT = 12
DEFAULT CHARACTER SET = utf8mb4;
...
(1, 'Active_RB1001', 100, 1, 1, 'yes'), -- Унікальний статус із штрихкодом
(2, 'Active_RB1002', 120, 2, 2, 'no'),
(3, 'Maintenance_RB1003', 150, 3, 3, 'yes'),
(4, 'Active_RB1004', 80, 4, 4, 'no'),
(5, 'Maintenance_RB1005', 130, 5, 5, 'yes'),
(6, 'Active_RB1006', 110, 6, 6, 'no'),
(7, 'Maintenance_RB1007', 90, 7, 7, 'yes'),
(8, 'Active_RB1008', 80, 3, 6, 'yes'),
(9, 'Active_RB1009', 80, 1, 7, 'no'),
(10, 'Active_RB1010', 90, 8, 8, 'yes'),
(11, 'Active_RB1011', 120, 9, 9, 'no'),
(12, 'Maintenance_RB1012', 110, 10, 10, 'yes'),
(13, 'Maintenance_RB1013', 95, 11, 1, 'no');
...
VALUES
    (1, 'Monitor Robot Alpha', 'Monitoring', 'High', 'Assigned', '2024-11-30', 'Ensure robot Alpha is operational.'),
    (2, 'Daily Performance Report', 'Reporting', 'Medium', 'In Progress', '2024-11-25', 'Submit a report on robot Beta performance.'),
    (3, 'Inspect Robot Sensors', 'Inspection', 'High', 'Assigned', '2024-11-28', 'Manually inspect sensors on robot Gamma.'),
    (4, 'Monitor Robot Delta', 'Monitoring', 'Low', 'In Progress', '2024-12-01', 'Ensure robot Delta is following its patrol route.'),
    (5, 'Weekly Summary Report', 'Reporting', 'Medium', 'Completed', '2024-11-27', 'Summarize activity of robot Epsilon.'),
    (6, 'Inspect Battery Levels', 'Inspection', 'High', 'Assigned', '2024-11-29', 'Check battery levels of robot Zeta.'),
    (7, 'Monitor Robot Eta', 'Monitoring', 'Medium', 'In Progress', '2024-11-26', 'Track movements of robot Eta.'),
    (8, 'Power Source Analysis', 'Other', 'Low', 'Assigned', '2024-12-02', 'Analyze alternative power sources for robot Theta.'),
    (9, 'Daily Sensor Health Check', 'Monitoring', 'High', 'Completed', '2024-11-28', 'Monitor sensor health of robot Iota.'),
    (10, 'Inspect Cameras', 'Inspection', 'High', 'Assigned', '2024-11-30', 'Verify operational status of cameras on robot Kappa.');
...
(7, '2024-09-13 16:00:00', 'Thermal system review', 'David Green', '2024-10-13 16:00:00'), 
(8, '2024-09-30 06:00:00', 'Battery performance tuning', 'John Smith', '2024-10-20 16:00:00'), 
(9, '2024-09-20 09:00:00', 'Communication protocol patch', 'Sara Lee', '2024-10-10 16:00:00'), 
(10, '2024-09-24 00:00:00', 'Propulsion unit inspection', 'Paul Walker', '2024-10-03 16:00:00'); 
